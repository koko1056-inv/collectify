-- Comprehensive fix for all critical security issues identified in security review
...
CREATE POLICY "Only current admins can view admin accounts"
...
CREATE POLICY "Only current admins can insert admin accounts"
ON public.admin_accounts
FOR INSERT
WITH CHECK (
  auth.uid() IN (
