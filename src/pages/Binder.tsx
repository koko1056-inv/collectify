...
export default function Binder() {
  ...
  if (selectedPageId) {
    return <BinderEditor pageId={selectedPageId} onClose={() => {
  ...
  return <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <main className="container mx-auto pt-20 px-4 py-8">
        <div className="max-w-6xl mx-auto space-y-4 md:space-y-6 animate-fade-in">
          ...
          {isLoadingPages ? <div className="flex items-center justify-center py-16">
              <div className="text-center space-y-3">
                <div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto" />
                <p className="text-sm text-muted-foreground">読み込み中...</p>
              </div>
            </div> : binderPages.length === 0 ? <Card className="p-8 md:p-12 text-center bg-white shadow-sm border-dashed border-2">
            ...
            </Card> : <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 md:gap-6">
              {binderPages.map(page => <Card key={page.id} className="overflow-hidden bg-white shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer group border border-gray-200 hover:border-primary/20">
                  <div className="aspect-[3/4] sm:aspect-[3/4] relative overflow-hidden bg-gradient-to-br from-gray-100 to-gray-50" style={{
              ...
              setIsPreviewMode(true);
                    ...
                    <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200 z-20">
                      <Button size="icon" variant="secondary" className="h-8 w-8 bg-white/90 hover:bg-white backdrop-blur-sm shadow-lg touch-manipulation" onClick={e => {
                  e.stopPropagation();
                  setSelectedPageId(page.id);
                  setIsPreviewMode(false);
                }}>
                      ...
                      <Button size="icon" variant="destructive" className="h-8 w-8 bg-white/90 hover:bg-destructive hover:text-white backdrop-blur-sm shadow-lg touch-manipulation" onClick={e => {
                  e.stopPropagation();
      ...
      <Button size="icon" className="fixed bottom-20 right-4 h-14 w-14 rounded-full shadow-lg hover:shadow-xl transition-all z-50" onClick={() => navigate("/binder/create")}>
