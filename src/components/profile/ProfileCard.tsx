...
export function ProfileCard({
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      const {
        data: profile,
        error
      } = await supabase.from("profiles").select("*").eq("id", effectiveUserId).single();
      if (error) {
        toast({
          variant: "destructive",
          title: "エラー",
  ...
  return <div className={`${isMobile ? 'bg-white' : 'bg-white p-6 rounded-lg shadow'}`}>
      ...
      {isOwnProfile && <div className="flex justify-center mb-4">
          <button onClick={() => setIsEditing(true)} className="text-center py-2 px-4 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors duration-200 text-sm font-bold">
            プロフィールを編集
          </button>
        </div>}

      <div className="mt-4">
        <ProfileBio bio={bio} isEditing={isEditing} onBioChange={e => setBio(e.target.value)} onEdit={() => setIsEditing(true)} onCancel={() => setIsEditing(false)} onSubmit={handleSubmit} saving={saving} isOwnProfile={isOwnProfile} />
      </div>

      <div className="mt-6">
        <ProfileFavorites userId={effectiveUserId} isEditing={isFavoritesEditing} onEditComplete={() => setIsFavoritesEditing(false)} />
      </div>
    </div>;
}