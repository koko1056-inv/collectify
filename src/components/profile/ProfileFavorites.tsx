...
export function ProfileFavorites({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from("user_items").select("*").eq("user_id", userId).order("created_at", {
        ascending: false
      });
  ...
  const handleSaveSelection = async () => {
    try {
      const {
        ...
        favorite_item_ids: selectedItems
      }).eq("id", userId);
  ...
  return <div className="space-y-4">
      <div className="flex items-center justify-between px-1">
        <h2 className="text-lg font-semibold px-[16px]">コレクション一覧</h2>
