...
export function AddItemsToGroupDialog({ isOpen, onClose, groupId }: AddItemsToGroupDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await addItemsToGroup(groupId, selectedItems);
      console.log("Add items result:", result);
      
      if (result.success) {
        toast.success(result.message || `${selectedItems.length}個のアイテムをグループに追加しました`);
        onClose();
      } else {
        toast.error(result.message || "グループへのアイテム追加に失敗しました");
      }
    } catch (error) {
